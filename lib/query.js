    ...
    switch (granule) {
        ...
        case 'v':
            ...
            if (area % 100 == 0 && area <= MAX_DID) {
                return `(e.vid BETWEEN ${area + 1} AND ${area + 99}) OR (e.vid BETWEEN ${area * 1000 + 1001} AND ${area * 1000 + 99999})`;
            }
            if (area <= MAX_DID) {
                if (tbName == LG) { // Select single electoral
                    let cid = area - area % 100;
                    return `e.ectr = area AND (e.vid = ${cid} OR e.vid BETWEEN ${cid * 1000 + 1} AND ${cid * 1000 + 999})`;
                }
                else {
                    return `e.ectr = area OR e.vid BETWEEN ${area * 1000 + 001} AND ${area * 1000 + 999}`;
                }
            };
            return `e.vid = ${area}`;
